document.addEventListener("DOMContentLoaded", function () {
  ...
  async function postComment(author, text) {
    addingCommentMessage = createMessage(
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 400) {
          alert(
            "Ошибка в отправленных данных. Проверьте и попробуйте еще раз."
          );
  ...
  commentForm.addEventListener("submit", function (event) {
    ...
    if (!navigator.onLine) {
      alert(
        "Вы потеряли подключение к интернету. Пожалуйста, восстановите его и попробуйте снова."
      );
      return;
    }
